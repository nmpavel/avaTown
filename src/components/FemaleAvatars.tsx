...
const FemaleAvatars = () => {
    const femaleAvatars = avatars.filter(obj=>obj.category==="Female");
    const [currentPage, setCurrentPage] = useState(1);
    const pageSize = 12; 
  
    const totalAvatars = femaleAvatars.length;
    const totalPages = Math.ceil(totalAvatars / pageSize);
    const startIndex = (currentPage - 1) * pageSize;
    const endIndex = startIndex + pageSize;
    const currentAvatars = femaleAvatars.slice(startIndex, endIndex);
